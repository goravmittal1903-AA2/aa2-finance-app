...
export async function POST(request: NextRequest) {
  const pending = await readGateToken(request.cookies.get('aa2_recovery_pending')?.value, 'recovery')
  if (!pending) return NextResponse.json({ error: 'Your recovery attempt has expired. Start again.' }, { status: 401 })
  const body = schema.safeParse(await request.json())
  if (!body.success) return NextResponse.json({ error: 'Enter the code from your email.' }, { status: 400 })
  ...
  response.cookies.set('aa2_recovery_pending', '', { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production', path: '/', maxAge: 0 })
  response.cookies.set('aa2_recovery_verified', await createGateToken('recovery', pending.email, data.user.id, 600), { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production', path: '/', maxAge: 600 })
  return response
}
